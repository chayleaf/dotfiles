{
  ...
  outputs =
    ...
    let
      ...
      priv =
        if builtins.pathExists ./private.nix then
          import ./private.nix { }
        else if builtins.pathExists ./private/default.nix then
          import ./private { }
        # workaround for git flakes not having access to non-checked out files
        else if builtins ? extraBuiltins.secrets then
          builtins.extraBuiltins.secrets
        # yes, this is impure, this is a last ditch effort at getting access to secrets
        else
          import /secrets/nixos { };
        ...
        if dev.${name} or false then
          (
            if input._type or null == "flake" then
              let
                inputs = input.inputs // {
                  self = (import /${devPath}/${name}/flake.nix).outputs inputs;
                };
              in
              { __toString = _: "/${toString devPath}/${name}"; } // inputs.self
            else
              /${devPath}/${name}
          )
        else
      ...
      overlay' =
        args: self: super:
        import (if args.pluginsOverlay or false then ./pkgs/nix-plugins-overlay.nix else ./pkgs) (
          {
            pkgs = super;
            pkgs' = self;
            lib = super.lib;
      ...
      mkPkgs =
        config:
        import (config.flake or nixpkgs) (
          builtins.removeAttrs config [ "flake" ]
          // {
    ...
    in
    {
      ...
      packages =
        lib.genAttrs
          ...
          (
            system:
            let
              self = overlay' { isOverlay = false; } (mkPkgs { inherit system; } // self) (
      ...
      nixosImages.router =
        let
          pkgs = mkPkgs { inherit (config.router-emmc) system; };
        in
        {
      ...
      hydraJobs = {
        ...
        router.${config.router-emmc.system} =
          self.nixosConfigurations.router-emmc.config.system.build.toplevel;
      ...
      nixosConfigurations = lib.flip builtins.mapAttrs config (
        hostname:
        args@{
          modules,
          nixpkgs ? { },
        ...
        (args.flake or base-inputs.nixpkgs).lib.nixosSystem {
          ...
          modules =
                ...
                { config, ... }:
                {
                  _module.args = {
                    pkgs-kernel = import inputs.nixpkgs-kernel {
                      inherit (args) system;
                      overlays = all-overlays ++ config.nixpkgs.overlays;
                    };
                  };
                }
              )
            ...
            ++ lib.optionals (home != { } && home.common.enableNixosModule or false) [
              ...
              {
                home-manager = builtins.removeAttrs (home.common or { }) [
                  "nixpkgs"
                  "nix"
                  "enableNixosModule"
                ];
        ...
        // builtins.listToAttrs (
          builtins.concatLists (
            lib.flip lib.mapAttrsToList config (
              ...
              let
                common' = builtins.removeAttrs (home.common or { }) [
                  "nix"
                  "nixpkgs"
                  "enableNixosModule"
                ];
                ...
                (
                  ...
                  lib.nameValuePair "${user}@${hostname}" (
                    inputs.home-manager.lib.homeManagerConfiguration (
                      common
                      // {
                        extraSpecialArgs = (common.extraSpecialArgs or { }) // {
                          inherit inputs;
                        };
                        modules =
                          homeConfig
