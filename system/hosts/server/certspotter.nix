...
in
{
  security.acme.certs = lib.mkIf config.services.certspotter.enable (
    lib.flip builtins.mapAttrs (lib.filterAttrs (k: v: v.enableACME) config.services.nginx.virtualHosts)
      (
        k: v: {
          postRun = ''
