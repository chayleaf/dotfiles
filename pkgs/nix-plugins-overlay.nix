...
let
  ...
  unpatchedNixForNixPlugins =
    if pkgs ? nixVersions.nix_2_25 then pkgs.nixVersions.nix_2_24 else pkgs.nixVersions.nix_2_18;
...
in
{
  ...
  nix-plugins =
    (pkgs.nix-plugins.override { nixVersions = patchedNixVersions; }).overrideAttrs
      (old: {
        # version = "13.0.0";
        patches = [
          /*
