...
let
  ...
  rofiI3 =
    if config.wayland.windowManager.sway.enable then
      pkgs.rofi.override { plugins = config.programs.rofi.plugins; }
    else
      pkgs.rofi;
  ...
  swaylockPre =
    if config.phone.enable then
      "${pkgs.coreutils}/bin/nohup ${pkgs.coreutils}/bin/env CTHULOCK_PASSWORD_FILE=/secrets/cthulock.pin "
    else
      "";
  ...
  swaylock =
    if config.phone.enable then
      "${pkgs.cthulock.override { withPatches = true; }}/bin/cthulock"
    else
    ...
    startup = [
      {
        command = toString (
          pkgs.writeShellScript "init-wm" ''
            ...
            ${pkgs.home-daemon}/bin/home-daemon daemon system76-scheduler ${lib.optionalString config.programs.notmuch.enable "email"} ${
  ...
  genKeybindings = (
    ...
    // {
      ...
      XF86AudioMicMute = lib.mkIf (
        !config.minimal
      ) "exec ${pkgs.pamixer}/bin/pamixer --default-source --toggle-mute";
    ...
    // (lib.filterAttrs
      ...
      (builtins.head (builtins.head default_options.config.type.getSubModules).imports)
...
in
{
  ...
  config = lib.mkMerge [
    (lib.mkIf (!config.minimal) {
      xdg.configFile."xdg-desktop-portal-wlr/config".source =
        (pkgs.formats.ini { }).generate "xdg-desktop-portal-wlr.ini"
          {
            screencast = {
              max_fps = 60;
              chooser_type = "simple";
    ...
    })
    {
      ...
      wayland.windowManager.sway = {
        ...
        config = commonConfig // {
          ...
          keybindings = genKeybindings options.wayland.windowManager.sway (
            ...
            */
            {
              "--inhibited --no-repeat --allow-other Scroll_Lock" = lib.mkIf (
                !config.minimal
              ) "exec ${pkgs.mumble}/bin/mumble rpc starttalking";
              "--inhibited --no-repeat --allow-other --release Scroll_Lock" = lib.mkIf (
                !config.minimal
              ) "exec ${pkgs.mumble}/bin/mumble rpc stoptalking";
              "${modifier}+c" =
                "exec ${rofiSway}/bin/rofi -show calc -no-show-match -no-sort -no-persist-history";
              "${modifier}+Print" = "exec ${grimshot}/bin/grimshot copy area";
              "${modifier}+${if modifier == "Mod1" then "Mod4" else "Mod1"}+Print" =
                "exec ${grimshot}/bin/grimshot copy window";
              "--locked XF86AudioRaiseVolume" = lib.mkIf (!config.minimal) (
                if config.phone.enable then
                  "exec ${pkgs.pamixer}/bin/pamixer --increase 5"
                else
                  "exec ${config.home.homeDirectory}/scripts/vol/raise.sh"
              );
              "--locked XF86AudioLowerVolume" = lib.mkIf (!config.minimal) (
                if config.phone.enable then
                  "exec ${pkgs.pamixer}/bin/pamixer --decrease 5"
                else
                  "exec ${config.home.homeDirectory}/scripts/vol/lower.sh"
              );
              "--locked XF86AudioMute" = lib.mkIf (!config.minimal) (
                if config.phone.enable then
                  "exec ${pkgs.pamixer}/bin/pamixer --toggle-mute"
                else
                  "exec ${config.home.homeDirectory}/scripts/vol/mute.sh"
              );
              "--locked --inhibited XF86AudioPlay" = lib.mkIf (
                !config.minimal
              ...
              "--locked --inhibited --release XF86PowerOff" =
                lib.mkIf config.phone.enable "exec ${pkgs.writeShellScript "power-key" ''
        ...
        extraSessionCommands =
          lib.optionalString config.wayland.windowManager.sway.vulkan ''
            export WLR_RENDERER=vulkan
          ''
          + ''
      ...
      services.swayidle = {
        ...
        events = [
          {
            event = "before-sleep";
            command = toString swaylock-start;
          }
      ...
      programs.rofi = {
        ...
        package = lib.mkIf config.wayland.windowManager.sway.enable (
          pkgs.rofi-wayland.override {
            rofi-unwrapped = pkgs.rofi-wayland-unwrapped.overrideAttrs (old: {
              patches =
                old.patches or [ ]
                ++ (lib.mapAttrsToList (k: v: ../../pkgs/rofi-wayland/${k}) (
                  builtins.readDir ../../pkgs/rofi-wayland
                ));
            });
          }
        );
